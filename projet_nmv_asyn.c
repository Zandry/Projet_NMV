...
static void nmv_kill(struct work_struct *wk){
	...
	strcpy(result, "Kill command success !");
	kill_cond = true; 
	wake_up(&wait_queue); 
}
...
{	
	switch(requete)
	{
		...
		case NMV_LSMOD:
			...
			/* Synchrone */
			wait_event(wait_queue, lsmod_cond); //flush_work(&(lsmod_command_execute.task));
			lsmod_cond = false; 
			asyn_cond = false; 
		...
		case NMV_LSMOD_ASYN:
			...
			kfree(result); 
			result = (char*)kmalloc(BUFFER_SIZE, GFP_KERNEL);
			strcpy(result, "Processing lsmod.... Please call NMV_ASYN to receive the result");
			/**/
